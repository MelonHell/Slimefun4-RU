...
public class ProgrammableAndroid extends SlimefunItem implements InventoryBlock, RecipeDisplayItem {
    ...
    public ProgrammableAndroid(ItemGroup itemGroup, int tier, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe) {
        ...
        new BlockMenuPreset(getId(), "Программируемый робот") {
            ...
            public void newInstance(BlockMenu menu, Block b) {
                menu.replaceExistingItem(15, new CustomItemStack(HeadTexture.SCRIPT_START.getAsItemStack(), "&aНачать/Продолжить"));
                ...
                menu.replaceExistingItem(17, new CustomItemStack(HeadTexture.SCRIPT_PAUSE.getAsItemStack(), "&4Пауза"));
                ...
                menu.replaceExistingItem(16, new CustomItemStack(HeadTexture.ENERGY_REGULATOR.getAsItemStack(), "&bЯдро памяти", "", "&8\u21E8 &7Нажми, чтобы открыть редактор скриптов"));
    ...
    public void openScript(Player p, Block b, String sourceCode) {
        ...
        menu.addItem(0, new CustomItemStack(Instruction.START.getItem(), Slimefun.getLocalization().getMessage(p, "android.scripts.instructions.START"), "", "&7\u21E8 &eЛКМ&7: Вернуться к роботу"));
        ...
        for (int i = 1; i < script.length; i++) {
            ...
            if (i == script.length - 1) {
                ...
                if (hasFreeSlot) {
                    menu.addItem(i, new CustomItemStack(HeadTexture.SCRIPT_NEW.getAsItemStack(), "&7> Добавить новую команду"));
                ...
                menu.addItem(slot, new CustomItemStack(Instruction.REPEAT.getItem(), Slimefun.getLocalization().getMessage(p, "android.scripts.instructions.REPEAT"), "", "&7\u21E8 &eЛКМ&7: Вернуться к роботу"));
                ...
                menu.addItem(i, new CustomItemStack(stack, Slimefun.getLocalization().getMessage(p, "android.scripts.instructions." + instruction.name()), "", "&7⇨ &eЛКМ&7: Изменить", "&7⇨ &eПКМ&7: Удалить", "&7⇨ &eShift + ПКМ&7: Продублировать"));
                menu.addMenuClickHandler(i, (pl, slot, item, action) -> {
    ...
    protected void openScriptDownloader(Player p, Block b, int page) {
        ChestMenu menu = new ChestMenu("Скрипты для роботов");
        ...
        menu.addItem(48, new CustomItemStack(HeadTexture.SCRIPT_UP.getAsItemStack(), "&eОпубликовать скрипт", "", "&6Нажми&7, чтобы опубликовать свой скрипт", "&7в базу сервера"));
        ...
        menu.addItem(53, new CustomItemStack(HeadTexture.SCRIPT_LEFT.getAsItemStack(), "&6> Назад", "", "&7Вернуться к роботу"));
    ...
    public void openScriptEditor(Player p, Block b) {
        ...
        menu.addItem(1, new CustomItemStack(HeadTexture.SCRIPT_FORWARD.getAsItemStack(), "&2> Редактировать скрипт", "", "&aИзменяет твой текущий скрипт"));
        ...
        menu.addItem(3, new CustomItemStack(HeadTexture.SCRIPT_NEW.getAsItemStack(), "&4> Создать новый скрипт", "", "&cУдаляет твой текущий скрипт", "&cи создает на замену чистый"));
        menu.addMenuClickHandler(3, (pl, slot, item, action) -> {
            openScript(pl, b, DEFAULT_SCRIPT);
            return false;
        });

        menu.addItem(5, new CustomItemStack(HeadTexture.SCRIPT_DOWN.getAsItemStack(), "&6> Загрузить скрипт", "", "&eЗагружает скрипт с сервера", "&e&oТы можешь его изменить"));
        menu.addMenuClickHandler(5, (pl, slot, item, action) -> {
            openScriptDownloader(pl, b, 1);
            return false;
        });

        menu.addItem(8, new CustomItemStack(HeadTexture.SCRIPT_LEFT.getAsItemStack(), "&6> Назад", "", "&7Вернуться к роботу"));
    ...
    protected void editInstruction(Player p, Block b, String[] script, int index) {
        ...
        menu.setEmptySlotsClickable(false);
        menu.addItem(9, new CustomItemStack(HeadTexture.SCRIPT_PAUSE.getAsItemStack(), "&fНичего не делать"), (pl, slot, item, action) -> {
            String code = deleteInstruction(script, index);
            setScript(b.getLocation(), code);
            openScript(p, b, code);
            return false;
